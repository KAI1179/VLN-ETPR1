...
**Key constraint**: The **cognitive maps** (CATEGORIES × ROWS × COLS) are **precomputed offline** and saved as `.npy` files (~35 MB/episode). These are **instruction-conditioned** `CognitiveGridMap` objects (not raw `GroundTruthGridMap`), generated by `data/prior/__main__.py` (`python -m prior`). The weighted-average embedding step is performed **at runtime** on GPU. No runtime dependency on `data.prior` or spaCy.
...
> **Correction**: Maps are stored **per episode**, not per scene. R2R train has ~10,819 episodes → ~379 GB; val_seen + val_unseen + test add ~2,500 more episodes → ~88 GB more. The "35 MB/scene" figure in the original plan was wrong.

| What is stored | Shape | Size/episode | R2R train (~10,819 eps) |
|----------------|-------|--------------|------------------------|
| Cognitive map (new) | (37, 500, 500) float32 | **35 MB** | **~370 GB** |
...
The per-episode cognitive map is **8–40× smaller** than pre-embedded maps, with the embedding method choice deferred to runtime. Storage can be reduced by compressing with `np.savez_compressed` (typically 2–5× for sparse maps) or by only precomputing maps for the splits needed (train + val only for development).
...
- `data/cognitive_maps/{scene_id}/episode_{episode_id}.npy` — shape `(37, 500, 500)` float32, contains `CognitiveGridMap.grid` (instruction-conditioned, path-filtered values in [0,1])
- `data/cognitive_maps/{scene_id}/episode_{episode_id}_meta.npz` — keys: `offset_x`, `offset_z`, `range_y` (range_y is `[None, None]` for single-floor/unknown-level scenarios)
...
| Map loading utils | `vlnce_baselines/models/etp_prior_gt/map_utils.py` | Load precomputed cognitive `.npy` by `(scene_id, episode_id)`, crop around agent |
...
- `data/cognitive_maps/{scene_id}/episode_{episode_id}_meta.npz` — `offset_x`, `offset_z`, `range_y`
...
> Note: some episodes may produce no `.npy` file (empty map). The trainer must handle missing maps gracefully (fall back to zero tensor).
...
class PrecomputedCognitiveMap:
    """Lightweight wrapper for a precomputed cognitive grid map (.npy)."""
...
def load_cognitive_map(precomputed_dir, scene_id, episode_id):
    ...
    Maps are stored as: {precomputed_dir}/{scene_id}/episode_{episode_id}.npy
...
OFFLINE (python -m prior  ←  data/prior/__main__.py):
  MP3D scene ──→ GroundTruthGridMap ──→ to_cognitive_map(instruction, positions)
                                    ...
                                    CognitiveGridMap (instruction-conditioned)
                                                  │
                              data/cognitive_maps/{scene_id}/episode_{id}.npy
                              (37 × 500 × 500)  ← ~35 MB each

RUNTIME (no data.prior dependency):
  Episode Start:
    (scene_id, episode_id) ──→ np.load("data/cognitive_maps/{scene}/episode_{id}.npy")
                                     │
                 PrecomputedCognitiveMap (lightweight wrapper)
                 [None if no .npy file — fall back to zero tensor]
...
Precomputed cognitive maps are ~35 MB each. With ~10,819 R2R train episodes, **pre-loading all maps is not feasible** (~370 GB). Options:
- **Lazy load + LRU cache** (recommended): Load maps on first access per `(scene_id, episode_id)`, keep the N most recent in memory (e.g., N=512 covers ~18 GB). Use `functools.lru_cache` on the load function.
...
- **Pre-load current batch**: At episode reset, load only the maps for the current batch of environments (e.g., 8–32 maps × 35 MB ≈ 280 MB–1.1 GB). Free them when episodes end.
...
2. **Map loading test**: Load `data/cognitive_maps/E9uDoFAP3SH/episode_238.npy`, verify shape `(37, 500, 500)`, crop to `(37, 101, 101)`, and check world-to-grid coordinate transform.
