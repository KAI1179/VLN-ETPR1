...
- `online_fusion` globally and recurrently updates the full cognitive-map token
  state from visited graph evidence without coordinate alignment, then lets all
  ghost nodes read the updated map for action prediction.
