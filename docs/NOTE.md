...
- 今日结论：精确 full-grid target 包含当前输入无法唯一恢复的 unseen-scene 几何，同时 predictor 存在可测量的 scene-specific overfit；mentioned category presence 较可靠，精确位置、unmentioned inventory 与轨迹方向尚不能可靠恢复。
- 下一步：修正 prompt 后对 full/mentioned-only target 短训，并做 prior/shuffle、同路径 paraphrase consistency 与下游导航因果对照。
