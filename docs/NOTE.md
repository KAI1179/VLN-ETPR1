...
- 与近期 peer-reviewed continuous R2R-CE `val_unseen` 结果只能作具名 caveat 的近似比较：`matched nav4` 的 SR `66.50` 高于本次核对行，SPL `55.31` 与 AwareVLN/GA-VLN/NavFoM 接近但低于 D3D-VLP `56.1`，因此不宣称 SOTA。本地是 Habitat 0.1.7、sliding、3 m、1,839 条固定分母，但 evaluator 为本地实现，且 sensor、训练数据、foundation model 与 seed 不同。离散 R2R、`test_unseen`、arXiv-only 和 privileged GT-map 行不混入主排名；旧 Try5 高分行是 GT cognitive-map upper bound，所以 `matched nav4` 只称当前最好的 verified matched LLM-Grid 结果。证据表、来源与 evaluator 边界见 [2026-08-10 记录](daily/2026-08-10.md#与近期-r2r-ce-方法的可比性)。
...
## 07/25（起点 oracle evidence 因果 screen）
