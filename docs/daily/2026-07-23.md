...
contract-v2 没有改善 unseen spatial quality；Raster IoU 与 Cell F1 的 CI 均跨零。seen spatial 点估计略升但同样不能与零区分，seen−unseen Raster IoU gap 反而从 0.0452 扩大到 0.0517。unseen object/region F1 和 direction cosine 明确下降；schema validity 的变化在两个 split 都不能与零区分。

因此错误 prompt 是真实的 contract defect，修正仍应保留，但本次 matched control 不支持“它是低 IoU 的主要原因”。单 seed/固定 greedy decode 不能说明修正对所有训练随机性都必然有害；可以可靠地说，它没有在本次严格对照中解释或解决 unseen full-grid 失败。
...
主要瓶颈是 input/target 的信息不匹配：当前输入不含场景观测，却要求 predictor 恢复 unseen house 的精确 full-grid 几何与未提及内容。scene-specific overfit 会放大 seen/unseen gap，但更像是在不可辨识 target 上拟合训练场景。错误 prompt 是独立 defect；matched 2-epoch control 已证明修正没有改善 unseen spatial quality，且 direction/category 指标部分下降，因此它不是当前低 IoU 的主要原因。
...
prompt 的 row/column 和 direction-vector 自然语言定义确有错误；训练 target、parser 和 evaluator 在数组层面仍相互一致，因此它不是 evaluator 评价错位。定义与共享输入字段已经修正并由 contract tests 固定。matched 2-epoch control 的 unseen Raster IoU delta 为 -0.0017 `[-0.0080, 0.0051]`，未改善 spatial quality；direction 与 object/region F1 反而下降。因此该 defect 需要保留修复，但不是当前低 IoU 的主要解释。
