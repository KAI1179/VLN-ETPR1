...
- [ ] 提交 matched 2-epoch prompt-contract control，并与旧 prompt 的 epoch 2 做完整 R2R predictor-quality 比较。
...
修正提交为 `a0bb323`。对照只改变 prompt contract 和共享输入字段名，保持 base model、R2R+RxR corpus、GT namespace、seed 42、LoRA、长度限制、batch、decode 与评价协议不变，并固定训练 2 epochs：
...
本地启动后在 manifest 序列化阶段按用户要求取消；8 个 worker 已退出，GPU 已释放。未完成产物保留为 `...contract-v2-e2-failed-local-20260723T0413Z`，正式输出路径仍为空，可由 Slurm launcher 原样创建。matched retraining 完成前，不判断错误 prompt 对低 IoU 的贡献大小。
...
以上两项联合支持一个较窄但更可靠的表述：instruction 确实提供 category、粗略 route geometry 和 direction 的 episode-specific signal；但它不足以唯一确定 unseen scene 的完整 absolute metric layout。结果不支持“predictor 完全没有学习输入”，也不支持“只要换一个更好的 predictor 就能恢复未观测 full grid”。prompt-contract matched control 仍用于独立量化错误 prompt 的贡献。
...
contract-v2 evaluator 完成后，用同一工具把旧 prompt epoch 2 作为 baseline：
...
python -m vlnce_baselines.models.etp_llm.llm_grid_paired_bootstrap \
  ...
  --output-dir outputs/llm_grid_eval/bootstrap-contract-v2-vs-old-epoch2-r1p5
```
...
下一步不应继续扩大模型或延长 full-grid 训练。先完成已准备的 prompt-contract matched 2-epoch control，量化该独立混淆因素；同时用现有 cache 完成 prior/shuffle 与同路径 paraphrase consistency，确认当前输出有多少 episode-specific input signal。之后再用相同初始化、数据和短训练预算依次比较 full-grid、mentioned-only absolute grid、mentioned-only start-centered/heading-normalized route corridor。该序列分别隔离 unmentioned content 与 absolute unseen-house geometry；只有在 `val_unseen` 上超过 input-agnostic prior、对 relevant-input shuffle 敏感且缩小 seen/unseen gap 的 target，才进入下游导航实验。
...
prompt 的 row/column 和 direction-vector 自然语言定义确有错误，可能妨碍 pretrained transfer 与 unseen 泛化；训练 target、parser 和 evaluator 在数组层面仍相互一致，因此它不是已观察到低 IoU 的直接评价错位。该定义与共享输入字段已经修正并由 contract tests 固定，但在 matched retraining 完成前，不对其贡献大小作判断。
...
2. **量化已经修正的 prompt contract**：完成相同 seed、data、训练预算与 decode 的 2-epoch matched control；不要把修正本身当作解决 input insufficiency 的方案。
