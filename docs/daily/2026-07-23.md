...
下一步不应继续扩大模型或延长 full-grid 训练。先统一固定 Python、NumPy、Torch/CUDA、LoRA 初始化与 sampler RNG，再按相同 run seed、数据和短训练预算依次比较 full-grid、mentioned-only absolute grid、mentioned-only start-centered/heading-normalized route corridor，分别隔离 unmentioned content 与 absolute unseen-house geometry。若这些可辨识 target 仍不足，再只改 cache generation，逐级加入起点 free-space、semantic inventory 和 sparse observed cells；只有在 `val_unseen` 上超过 prior、对 relevant-input shuffle 敏感且缩小 seen/unseen gap 的方案，才进入下游导航实验。
...
2. **已量化修正后的 prompt contract**：相同 sampler seed、data、训练预算与 decode 的 2-epoch control 未改善 unseen spatial quality，但未覆盖训练 RNG；后续实验继续使用正确 contract，不再把 prompt engineering 作为解决 input insufficiency 的主线。
3. **先修复复现性，再做最小 matched target ablation**：统一固定 Python、NumPy、Torch/CUDA、LoRA 初始化与 sampler RNG，再以 paired run seeds 比较 full-grid → mentioned-only absolute grid → mentioned-only start-centered/heading-normalized route corridor。第一步隔离 unmentioned content，第二步隔离 absolute scene geometry。
