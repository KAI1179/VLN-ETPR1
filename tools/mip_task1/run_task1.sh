...
CHECKS=$REPO_ROOT/tools/mip_task1/checks
...
step_A0() {
  ...
  if [ ! -f "$REPORT" ]; then
    { echo "# Task 1 Report"; echo; echo "generated by tools/mip_task1/run_task1.sh, started $(ts) on $(hostname)"; } > "$REPORT"
  fi
...
log "done. report: $REPORT — now run: bash $REPO_ROOT/tools/mip_task1/collect.sh"
