...
def parse_log_file(log_file_path):
            ...
            if "validation finished" in line and context and task:
                if task == "mlm":
                    ...
                    if acc_match:
                        ...
                        validation_results[current_step][key] = float(
                            acc_match.group(1)
                        )
    ...
    complete_results = {
        ...
        if "r2r_mlm_acc" in data
        and "r2r_sap_gacc" in data
        and "rxr_mlm_acc" in data
        and "rxr_sap_gacc" in data
    }
...
def calculate_performance(validation_results, mode="total"):
    ...
    for step, data in validation_results.items():
        ...
        if mode == "r2r":
            ...
            score = (
                data.get("r2r_mlm_acc", 0)
                + data.get("r2r_sap_gacc", 0)
                + data.get("rxr_mlm_acc", 0)
                + data.get("rxr_sap_gacc", 0)
            )
...
def main():
    ...
    for ckpt in top_5_checkpoints:
        if mode == "r2r":
            print(
                f"Step: {ckpt['step']}, r2r_mlm_acc: {ckpt['r2r_mlm_acc']:.2f}, r2r_sap_gacc: {ckpt['r2r_sap_gacc']:.2f}, Score: {ckpt['score']:.2f}"
            )
        elif mode == "rxr":
            print(
                f"Step: {ckpt['step']}, rxr_mlm_acc: {ckpt['rxr_mlm_acc']:.2f}, rxr_sap_gacc: {ckpt['rxr_sap_gacc']:.2f}, Score: {ckpt['score']:.2f}"
            )
        else:
            print(
                f"Step: {ckpt['step']}, r2r_mlm_acc: {ckpt['r2r_mlm_acc']:.2f}, r2r_sap_gacc: {ckpt['r2r_sap_gacc']:.2f}, rxr_mlm_acc: {ckpt['rxr_mlm_acc']:.2f}, rxr_sap_gacc: {ckpt['rxr_sap_gacc']:.2f}, Score: {ckpt['score']:.2f}"
            )
