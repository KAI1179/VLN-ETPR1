...
def process_features(proc_id, out_queue, scanvp_list, args):
    ...
    with h5py.File(args.img_db, "r") as f:
        for scan_id, viewpoint_id in scanvp_list:
            ...
            images = torch.stack([img_transforms(image) for image in images], 0).to(
                device
            )
            fts, logits = [], []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img_db", default="pretrain_src/img_features/habitat_256x256_vfov60_bgr.hdf5"
    )
    parser.add_argument("--out_image_logits", action="store_true", default=False)
    parser.add_argument(
        "--output_file",
        default="pretrain_src/img_features/CLIP-ViT-B-32-views-habitat.hdf5",
    )
