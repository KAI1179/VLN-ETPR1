...
def get_img(proc_id, out_queue, scanvp_list, args):
    ...
    for scan_id, viewpoint_id in scanvp_list:
        if scan_id != pre_scan:
            ...
            habitat_sim = HabitatUtils(
                f"data/scene_datasets/mp3d/{scan_id}/{scan_id}.glb",
                int(0),
                60,
                HEIGHT,
                WIDTH,
...
def build_img_file(args):
    ...
    for proc_id in range(num_workers):
        ...
        process = mp.Process(
            target=get_img, args=(proc_id, out_queue, scanvp_list[sidx:eidx], args)
        )
        process.start()
    ...
    with h5py.File(args.output_file, "w") as outf:
        while num_finished_workers < num_workers:
            ...
            if res is None:
                ...
                if args.img_type == "rgb":
                    outf.create_dataset(
                        key, data=images, dtype="uint8", compression="gzip"
                    )
                elif args.img_type == "depth":
                    outf.create_dataset(
                        key, data=images, dtype="float32", compression="gzip"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--connectivity_dir", default="precompute_img_features/connectivity"
    )
    ...
    if args.img_type == "rgb":
        args.output_file = (
            f"pretrain_src/img_features/habitat_{HEIGHT}x{WIDTH}_vfov{VFOV}_bgr.hdf5"
        )
    elif args.img_type == "depth":
        args.output_file = (
            f"pretrain_src/img_features/habitat_{HEIGHT}x{WIDTH}_vfov{VFOV}_depth.hdf5"
        )
    else:
