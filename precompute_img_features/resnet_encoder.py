...
if __name__ == '__main__':
    observation_space = {"depth": Box(low=0.0, high=1.0, shape=(256,256,1), dtype='float32')}
    # print(observation_space)
    net = VlnResnetDepthEncoder(
        observation_space,
        output_size=256,
        checkpoint='/data/andong/vlnce_dataset/data/ddppo-models/gibson-2plus-resnet50.pth',
        backbone='resnet50',
        spatial_output=False,
    )
    inputs = {"depth": torch.randn(2,256,256,1)}
    outs = net(inputs)
