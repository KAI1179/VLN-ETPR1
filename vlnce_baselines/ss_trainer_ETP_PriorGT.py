...
class RLTrainer(BaseVLNCETrainer):
    ...
    def _teacher_action_new(self, batch_gmap_vp_ids, batch_no_vp_left, is_train):
        ...
        for i, (gmap_vp_ids, gmap, no_vp_left) in enumerate(
            ...
            curr_dis_to_goal = self.envs.call_at(
                i, "current_dist_to_goal", {"is_train": is_train}
            )
            if curr_dis_to_goal < 1.5:
                ...
                if no_vp_left:
                    ...
                    ghost_dis_to_goal = [
                        self.envs.call_at(
                            i, "point_dist_to_goal", {"pos": p[1], "is_train": is_train}
                        )
    ...
    def train(self):
        ...
        for idx in range(start_iter, total_iter, log_every):
            ...
            if self.local_rank < 1:
                ...
                if self._refiner_enabled():
                    p0_cells, refined_cells = self._refiner_active_cells
                    logger.info(
                        "[Refiner active] enabled=True p0_active_cells=%d "
                        "refined_active_cells=%d",
                        int(p0_cells),
                        int(refined_cells),
                    )
                else:
                    logger.info("[Refiner active] enabled=False")
                ...
                if checkpoint_interval <= 0 or (
                    cur_iter % checkpoint_interval == 0 or cur_iter == total_iter
                ):
    ...
    def _update_refined_cognitive_maps(self, observations, cognitive_maps):
        ...
        if self.local_rank < 1:
            self._refiner_active_cells = (
                (p0[0] >= 0.5).any(dim=0).sum(),
                (refined[0] >= 0.5).any(dim=0).sum(),
            )
