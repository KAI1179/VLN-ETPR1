...
class VLNCEDaggerEnv(habitat.RLEnv):
    ...
    def current_dist_to_goal(self, is_train):
        ...
        if is_train:
            init_distance = self._env.sim.geodesic_distance(
                init_state.position,
                self._env.current_episode.goals[-1].position,
            )
        else:
            init_distance = self._env.sim.geodesic_distance(
                init_state.position,
    ...
    def current_dist_to_refpath(self, path):
        ...
        circle_dists = []
        for pos in path:
            circle_dists.append(self._env.sim.geodesic_distance(current_pos, pos))
    ...
    def ghost_dist_to_ref(self, ghost_vp_pos, ref_path):
        ...
        if circle_bool.sum() == 0:  # no gt point within 3.0m
            ...
            cand_idxes = np.where(
                circle_bool * (np.arange(0, len(ref_path)) >= progress)
            )[0]
            if len(cand_idxes) == 0:
                ...
                compare = (
                    np.array(
                        list(range(cand_idxes[0], cand_idxes[0] + len(cand_idxes)))
                    )
                    == cand_idxes
                )
                if np.all(compare):
    ...
    def cand_dist_to_subgoal(self, angle: float, forward: float, sub_goal: Any):
        r"""get resulting distance to goal by executing
        a candidate action"""
        ...
        post_distance = (
            self._env.sim.geodesic_distance(
                post_state.position,
                sub_goal,
            )
            + dis
    ...
    def turn(self, ang, vis_info):
        ...
        uni_l = (
            self._env.sim.get_agent(0).agent_config.action_space[act_l].actuation.amount
        )
    ...
    def single_step_control(self, pos, tryout, vis_info):
        ...
        uni_f = (
            self._env.sim.get_agent(0).agent_config.action_space[act_f].actuation.amount
        )
        agent_state = self._env.sim.get_agent_state()
        ang, dis = calculate_vp_rel_pos(
            agent_state.position, pos, heading_from_quaternion(agent_state.rotation)
        )
    ...
    def get_plan_frame(self, vis_info):
        ...
        frame = cv2.copyMakeBorder(
            frame, 6, 6, 5, 5, cv2.BORDER_CONSTANT, value=(255, 255, 255)
        )
    ...
    def step(self, action, vis_info, *args, **kwargs):
        ...
        if self.video_option and done:
            ...
            generate_video(
                ...
                episode_id=self._env.current_episode.episode_id,
                scene_id=self._env.current_episode.scene_id.split("/")[-1].split(".")[
                    -2
                ],
                checkpoint_idx=0,
