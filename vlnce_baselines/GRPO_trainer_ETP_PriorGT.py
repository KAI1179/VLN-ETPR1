...
class RLTrainer(BaseVLNCETrainer):
    def __init__(self, config=None):
        ...
        print("config.GRPO:\n", config.GRPO)
        print(
            f"GRPO params: grpo_epsilon {self.grpo_epsilon}, grpo_beta {self.grpo_beta}, max_grad_norm {self.max_grad_norm}, grpo_update_epochs {self.grpo_update_epochs} \
    ...
    def _set_config(self):
        ...
        print(
            f"init camera information: resize_config:{resize_config}, crop_config:{crop_config}, new_camera_heading:{camera_orientations}"
        )
    ...
    def _init_envs(self):
        ...
        self.config.TASK_CONFIG.SEED = (
            self.config.TASK_CONFIG.SEED + self.local_rank * 10
        )
        self.config.freeze()
    ...
    def _initialize_policy(
        ...
        not_trainable_parameters = [
            p for p in self.policy.parameters() if not p.requires_grad
        ]
        trainable_parameters = [
            (n, p) for n, p in self.policy.named_parameters() if p.requires_grad
        ]
        ...
        if load_from_ckpt:
            ...
            if (
                ...
                "module" not in list(ckpt_dict["state_dict"].keys())[0]
                and self.config.GPU_NUMBERS > 1
            ):
            ...
            if incompatible_keys.missing_keys:
                print(
                    "The following network layers exist in the model but are missing in the weight file (initial values will be used):"
                )
                for key in sorted(incompatible_keys.missing_keys):
                    print(f"  - {key}")
            else:
                print(
                    "All network layers present in the model were found in the weight file."
                )
    ...
    def _nav_gmap_variable(self, cur_vp, cur_pos, cur_ori, task_type):
        ...
        for i, gmap in enumerate(self.gmaps):
            ...
            batch_gmap_task_embeddings.append(
                torch.full_like(gmap_step_ids_tensor, task_type)
            )
        ...
        batch_gmap_task_embeddings = pad_sequence(
            batch_gmap_task_embeddings, batch_first=True
        ).cuda()
        ...
        for i in range(bs):
            gmap_pair_dists[i, : batch_gmap_lens[i], : batch_gmap_lens[i]] = (
                ...
                for k, v_list in logs.items():
                    ...
                    if self.local_rank < 1:
                        global_sum, global_count = (
                            metric_tensor[0].item(),
                            metric_tensor[1].item(),
                        )
                        if global_count > 0:
    ...
    def update(self):
        ...
        advantages_all_ep_all_samples = [
            [0.0] * self.initial_num_envs for _ in range(self.config.GRPO.sample_num)
        ]
        ...
        for original_env_idx in range(self.initial_num_envs):
            ...
            if len(rewards_for_this_slot) > 1:
                ...
                for s_idx in range(self.config.GRPO.sample_num):
                    reward_val = self.data_buffer[s_idx]["reward"][original_env_idx]
                    if reward_val is not None:
                        advantages_all_ep_all_samples[s_idx][original_env_idx] = (
                            reward_val - mean_r
                        ) / (std_r + 1e-8)

        all_spls_in_buffer = []
        for s_idx in range(self.config.GRPO.sample_num):
            all_spls_in_buffer.extend(
                [r for r in self.data_buffer[s_idx]["reward"] if r is not None]
            )
        ...
        for epoch in range(self.grpo_update_epochs):
            ...
            for s_idx in range(self.config.GRPO.sample_num):
                current_sample_trajectory_steps_data = self.data_buffer[s_idx][
                    "data_buffer"
                ]

                initial_txt_embeds_cuda = self.data_buffer[s_idx][
                    "initial_txt_embeds"
                ].to(self.device, non_blocking=True)
                initial_txt_masks_cuda = self.data_buffer[s_idx][
                    "initial_txt_masks"
                ].to(self.device, non_blocking=True)
                ...
                with autocast(enabled=self.enable_amp):
                    for step_data in current_sample_trajectory_steps_data:
                        with autocast(enabled=False):
                            ...
                            for key, value in nav_inputs_cpu.items():
                                if isinstance(value, torch.Tensor):
                                    nav_inputs_cuda[key] = value.to(
                                        self.device, non_blocking=True
                                    )
                                else:
                                    nav_inputs_cuda[key] = value
                            txt_embeds_for_step = initial_txt_embeds_cuda[
                                active_indices_in_original_batch
                            ]
                            txt_masks_for_step = initial_txt_masks_cuda[
                                active_indices_in_original_batch
                            ]
                            ...
                            if "map_tokens" in step_data:
                                ...
                                map_tokens_for_step, map_token_masks_for_step = (
                                    select_replay_map_inputs(
                                        step_map_tokens,
                                        step_map_token_masks,
                                        active_indices_in_original_batch,
                                    )
                                )
                                nav_inputs_cuda["map_tokens"] = map_tokens_for_step.to(
                                    self.device, non_blocking=True
                                )
                                nav_inputs_cuda["map_token_masks"] = (
                                    map_token_masks_for_step.to(
                                        self.device, non_blocking=True
                                    )
                                )
                        ...
                        if self.need_ref_policy:
                            with torch.no_grad():
                                ref_policy_outputs = self.ref_policy.net(
                                    **nav_inputs_cuda
                                )
                        ...
                        with autocast(enabled=False):
                            ...
                            current_log_probs_taken_action = current_log_probs.gather(
                            ...
                            step_advantages_for_active_envs = torch.tensor(
                                [
                                    advantages_all_ep_all_samples[s_idx][orig_idx]
                                    for orig_idx in active_indices_in_original_batch
                                ],
                                device=self.device,
                                dtype=torch.float32,
                            )
                            ...
                            if self.grpo_update_epochs == -1:
                                old_log_probs_taken_action = (
                                    current_log_probs_taken_action.detach()
                                )
                            else:
                                old_log_probs_taken_action = torch.log(
                                    old_probs_at_sampling_cpu.to(self.device)
                                    .gather(1, taken_actions_cuda.unsqueeze(1))
                                    .squeeze(1)
                                    + 1e-9
                                )
                            ...
                            if self.need_ref_policy:
                                ratio_ref_over_current = torch.exp(
                                    ref_log_probs_taken_action_no_grad
                                    - current_log_probs_taken_action
                                )
                            ...
                            batch_total_policy_loss_for_this_sample += (
                                policy_loss_this_step
                            )
                            if self.need_ref_policy:
                ...
                if num_valid_steps_for_this_sample > 0:
                    avg_policy_loss_for_sample = (
                        batch_total_policy_loss_for_this_sample
                        / num_valid_steps_for_this_sample
                    )
                    if self.need_ref_policy:
                        avg_kl_loss_for_sample = (
                            batch_total_kl_loss_for_this_sample
                            / num_valid_steps_for_this_sample
                        )

                    if self.need_ref_policy:
                        total_loss_for_this_sample = (
                            avg_policy_loss_for_sample
                            + self.grpo_beta * avg_kl_loss_for_sample
                        )
                    else:
                        total_loss_for_this_sample = avg_policy_loss_for_sample
                    scaled_loss_for_this_sample = total_loss_for_this_sample

                    if (s_idx < self.config.GRPO.sample_num - 1) and (
                        self.world_size > 1
                    ):
                    ...
                    accumulated_policy_loss_this_epoch += (
                        avg_policy_loss_for_sample.item()
                    )
                    if self.need_ref_policy:
            ...
            if num_samples_processed_this_epoch == self.config.GRPO.sample_num:
                ...
                total_policy_loss_across_epochs += (
                    accumulated_policy_loss_this_epoch
                    / num_samples_processed_this_epoch
                )
                if self.need_ref_policy:
                    total_kl_loss_across_epochs += (
                        accumulated_kl_loss_this_epoch
                        / num_samples_processed_this_epoch
                    )
                if self.need_ref_policy:
                    total_combined_loss_across_epochs += (
                        accumulated_policy_loss_this_epoch
                        + self.grpo_beta * accumulated_kl_loss_this_epoch
                    ) / num_samples_processed_this_epoch
                else:
                    total_combined_loss_across_epochs += (
                        accumulated_policy_loss_this_epoch
                ...
                logger.info(
                    f"Epoch {epoch + 1}/{self.grpo_update_epochs}: actual samples != GRPO.sample_num."
                )

        if actual_epochs_processed > 0:
            self.logs["policy_loss"].append(
                total_policy_loss_across_epochs / actual_epochs_processed
            )
            if self.need_ref_policy:
                self.logs["kl_loss"].append(
                    total_kl_loss_across_epochs / actual_epochs_processed
                )
            self.logs["total_loss"].append(
                total_combined_loss_across_epochs / actual_epochs_processed
            )

            if total_processed_actions_for_ratio > 0:
                unclipped_ratio = (
                    total_unclipped_actions / total_processed_actions_for_ratio
                )
            ...
            logger.info(
                "No valid data processed across all epochs. Skipping logging of losses."
            )
    ...
    def sample_data(self, sample_num):
        ...
        for i in range(sample_num):
            ...
            if i == 0:
                ...
                observations = self.envs.call(
                    ["reset_current_episode"] * self.envs.num_envs
                )
    ...
    def sample_once(self, initial_obs):
        ...
        for stepk in range(self.max_len):
            ...
            if current_map_tokens is not None:
                ...
                data_this_stepk["map_token_masks"] = (
                    current_map_token_masks.detach().cpu()
                )
            ...
            if self.config.MODEL.task_type == "r2r":
                for i in range(self.envs.num_envs):
                    ...
                    metric["success"] = 1.0 if distances[-1] <= 1.5 else 0.0
                    metric["oracle_success"] = 1.0 if (distances <= 1.5).any() else 0.0
                    ...
                    data_this_sample["reward"][not_done_index[i]] = (
                        metric["spl"] + metric["success"] + distance_to_goal_reward
                    )
                ...
                for i in range(self.envs.num_envs):
                    ...
                    distances = np.array(info["position_train"]["distance"])
                    gt_length = max(
                        self.gt_data[str(ep_id)]["forward_steps"] * 0.25, distances[0]
                    )
                    metric = {}
                    metric["distance_to_goal"] = distances[-1]
                    metric["success"] = 1.0 if distances[-1] <= 1.5 else 0.0
            ...
            if sum(dones) > 0:
                for i in reversed(list(range(self.envs.num_envs))):
                    if dones[i]:
                        ...
                        all_txt_task_encoding = torch.cat(
                            (all_txt_task_encoding[:i], all_txt_task_encoding[i + 1 :]),
                            dim=0,
                        )
                        all_txt_masks = torch.cat(
                            (all_txt_masks[:i], all_txt_masks[i + 1 :]), dim=0
                        )
