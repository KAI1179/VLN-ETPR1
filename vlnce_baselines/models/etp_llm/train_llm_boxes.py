...
def generate_navigation_cache(
    ...
    """Generate LLM-Navigation prediction text and cognitive-map caches."""
    import torch
    ...
    _write_navigation_cache_manifest(split_dir, args, system_prompt)
    ...
    progress_loader = _progress(
        loader,
        desc="generate LLM-Navigation cache",
        quiet=bool(getattr(args, "quiet", False)),
        total=_batch_count(dataset, args.batch_size),
    )
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    cache_parser = subparsers.add_parser(
        "cache",
        help="Generate LLM-Navigation cognitive-map cache",
    )
    _add_common_args(cache_parser)
    _add_cache_args(cache_parser)
...
def main(argv: Optional[Sequence[str]] = None) -> Dict[str, float]:
    ...
    if args.mode == "cache":
        ...
        return generate_navigation_cache(
            model,
            tokenizer,
...
def _add_cache_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help="VLN-CE split to generate into the LLM-Navigation cache.",
    )
    parser.add_argument(
        "--cache-dir",
        default="",
